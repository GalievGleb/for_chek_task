...
class TestBookings:
    """Класс тестов для работы с букингом через API. Содержит тесты на создание, изменение, удаления,
    а так же проверки букингов"""
    ...
    @pytest.mark.positive
    def test_updt_booking(self, booking_data, auth_session, create_booking):
        ...
        updated_data["additionalneeds"] = "Breakfast, lunch and dinner"
        updated_data["bookingdates"] = {
            "checkin": "2024-04-07",
            "checkout": "2024-04-14"
        }

        updt_booking = auth_session.put(f"{BASE_URL}/booking/{booking_id}", json=updated_data)
        ...
        assert booking_data_response['bookingdates']['checkout'] == booking_data['bookingdates'][
            'checkout'], "Дата выезда не совпадает"
        assert updt_booking.json()["additionalneeds"] == "Breakfast, lunch and dinner"
        assert updt_booking.json()["bookingdates"] == {
            "checkin": "2024-04-07",
            "checkout": "2024-04-14"
        }
    ...
    def test_empty_booking(self, auth_session):
        """Создает бронирование не заполняя ни одного поля"""
        create_empty_booking = auth_session.post(f"{BASE_URL}/booking", json= {
        "firstname": "",
        "lastname": "",
        "totalprice": "",
        "depositpaid": "",
